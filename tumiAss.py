...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 10 18:10:01 2022
...
df = pd.read_csv('International_Breweries.csv')
...
def plot_profit_trend(df, date, profit):
    '''
        ...
        Returns:
            grapgh => lineplot
    '''
    profit_trend = df.groupby(date).sum()[profit]
    date = profit_trend.index
    profit = profit_trend.to_list()
 ...
 def plot_profit_trend(df, date, profit):
    '''
        ...
        Args:
            ...
            date => str, representing the name of date column in dataframe
            profit => str, representing the name of date column in dataframe
            brands => list, containing brands to plot
        Returns:
            grapgh => lineplot
    '''
...
plot_profit_trend(df, 'YEARS', 'PROFIT')
...
def plot_quantity_by_region(df, region, quantity):
    ...
    #plt.figure(figsize = (20,10)) # set size of line plot
    plt.pie(quantity, colors=colors, autopct='%1.1f%%', startangle=140)
